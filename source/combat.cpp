...
for(int32_t i = SLOT_FIRST; i <= SLOT_LAST; i++){
							if((tmpItem = player->getInventoryItem((slots_t)i))){
								...
								const ItemType& it = Item::items[tmpItem->getID()];
								if(it.abilities.increasemagicpercent != 0)
								{
								  int increasemagicpercent =  it.abilities.increasemagicpercent; 
									  minChange += (minChange * increasemagicpercent)/100; 
									  maxChange += (maxChange * increasemagicpercent)/100; 
								}
							}
						}
		}			
