...
int32_t Player::getStepSpeed() const
{
	if(hasFlag(PlayerFlag_SetMaxSpeed))
		return 9000;
	else if(getSpeed() > PLAYER_MAX_SPEED){
    ...
    if (g_game.getWorldType() == WORLD_TYPE_PVP_ENFORCED && g_config.getNumber(ConfigManager::RATE_EXPERIENCE_PVP) > 0) {
        ...
        if(attackerPlayer && attackerPlayer != this && skillLoss) {
                ...
                */
                ...
                if ( attackerLevel >= std::floor(victimLevel * 0.50) ) {
                    ...
                    std::stringstream ss; ss << "You killed "<< getName() <<" and received "<< (uint64_t)(result * 0.50) << " experience points.";
                    attackerPlayer->sendTextMessage(MSG_EVENT_DEFAULT, ss.str());
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	sendTextMessage(MSG_STATUS_WARNING, msg.str());
		...
		lastSkullTime = lastSkullTime + g_config.getNumber(ConfigManager::FRAG_TIME);

	int32_t unjustKills = IOPlayer::instance()->getPlayerUnjustKillCount(this, UNJUST_KILL_PERIOD_DAY);
	if(lastSkullTime >= g_config.getNumber(ConfigManager::RED_UNJUST) * g_config.getNumber(ConfigManager::FRAG_TIME)){
		setSkull(SKULL_RED);
		g_game.updateCreatureSkull(this);
	}
	if (lastSkullTime >= g_config.getNumber(ConfigManager::BAN_UNJUST) * g_config.getNumber(ConfigManager::FRAG_TIME))
	{
		if (g_bans.addPlayerBan(getName(), std::time(NULL) + g_config.getNumber(ConfigManager::BAN_LENGTH), 1, "Automatic", "", 28, ACTION_BANISHMENT))
		{
...
{
	if(lastSkullTime <= 500){
		lastSkullTime = 0;
    }
	if(lastSkullTime - ticks > 0)
		lastSkullTime = lastSkullTime - ticks;	
	if(getSkull() == SKULL_NONE && lastSkullTime >= g_config.getNumber(ConfigManager::RED_UNJUST) * g_config.getNumber(ConfigManager::FRAG_TIME)){
		setSkull(SKULL_RED);
		g_game.updateCreatureSkull(this);
	}
	if(lastSkullTime < 1000 && !hasCondition(CONDITION_INFIGHT) && getSkull() == SKULL_RED){
			lastSkullTime = 0;
			setSkull(SKULL_NONE);
			clearAttacked();
			g_game.updateCreatureSkull(this);
...
void Player::broadcastLoot(Creature* creature, Container* corpse)
{
	...
	if(getParty())
		getParty()->broadcastPartyMessage(MSG_INFO_DESCR, os.str(), false);
	else
	{
