...
Item* Monster::createCorpse()
{
    ...
    if(corpse){
        ...
        if(!killers.empty() && (*killers.rbegin()).isCreatureKill()){
            ...
            if(corpseOwner != NULL){
                corpse->setOwner(corpseOwner->getID());
            }
        }
    }
...
void Monster::dropLoot(Container* corpse)
{
    if(corpse && lootDrop){
		 mType->createLoot(corpse);
          Player* killer = g_game.getPlayerByID(corpse->getOwner());
        if(killer && (killer->getPremiumDays() > 0 || (killer->getParty() && killer->getParty()->isPremium()))) {
