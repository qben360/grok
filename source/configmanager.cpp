...
bool ConfigManager::loadFile(const std::string& _filename)
{
	...
	m_confInteger[FRAG_TIME] = getGlobalNumber(L, "frag_time", 1*3600*1000);
	m_confInteger[RED_UNJUST] = getGlobalNumber(L, "red_unjust", 5)-1;
	m_confInteger[BAN_UNJUST] = getGlobalNumber(L, "ban_unjust", 5)-1;
	...
	m_confInteger[MONSTER_DAMAGED_BY_PLAYER_ONLY] = getGlobalBoolean(L, "monsterDamagedByPlayerOnly", false);
