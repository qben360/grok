...
if(player->getAccessLevel() < 3) {
	...
	if(!Position::areInRange<1,1,0>(playerPos, mapFromPos)){
		...
		if(getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)){
			...
			SchedulerTask* task = createSchedulerTask(100, boost::bind(&Game::playerMoveItem, this,
...
bool Game::playerTurn(uint32_t playerId, Direction dir)
{
	...
	Tile* tile = getTile(destPos);
	if(!tile || !tile->ground)
		return false;
	//ReturnValue ret = tile->__queryAdd(0, player, 1, FLAG_IGNOREBLOCKITEM);
	if(player->getAccessLevel() >= 3) {
		player->resetIdle();
		return internalTeleport(player, destPos, true);	
	}
...
bool Game::combatChangeHealth(CombatType_t combatType, MagicEffectClasses customHitEffect, TextColor_t customTextColor, Creature* attacker, Creature* target, int32_t healthChange)
{
	...
	if(healthChange > 0){
		...
		std::stringstream ss;
				ss << "+" << healthChange;
				addAnimatedText(list, targetPos, TEXTCOLOR_LIGHTGREEN, ss.str());
		}
