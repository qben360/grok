...
void LuaScriptInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "getInFightTicks", LuaScriptInterface::luaGetInFightTicks);
	...
	lua_register(m_luaState, "getPlayerManaSpent", LuaScriptInterface::luaGetPlayerSpentMana);
...
int LuaScriptInterface::internalGetPlayerInfo(lua_State *L, PlayerInfo_t info)
{
	...
	switch(info){
		...
		{
			lua_pushnumber(L, player->manaSpent);
			return 1;
		}
		...
		{
			int vocation = player->getVocationId();
			if (vocation > 4)
				vocation = vocation - 4;
			lua_pushnumber(L, vocation);
			return 1;
		}
		case PlayerInfoSoul:
		{
...
int LuaScriptInterface::luaGetPlayerSpentMana(lua_State *L)
{
...
int LuaScriptInterface::luaPlayerLearnInstantSpell(lua_State *L)
{
	...
	InstantSpell* spell = g_spells->getInstantSpell(spellName);
	if(!spell){
...
//azuu
int LuaScriptInterface::luaGetInFightTicks(lua_State* L)
{
    ScriptEnviroment* env = getScriptEnv();
    Creature* creature = env->getCreatureByUID(popNumber(L));
    Condition* condition = creature->getCondition(CONDITION_INFIGHT, CONDITIONID_DEFAULT, 0);
...
bool LuaScriptInterface::getArea(lua_State *L, std::list<uint32_t>& list, uint32_t& rows)
{
	if(lua_type(L, -1) != LUA_TTABLE){
		reportError(__FUNCTION__, "Variable passed to [LuaScriptInterface::getArea] is not a table.");
		return false;
	}
	rows = 0;
	uint32_t i = 0, j = 0;
