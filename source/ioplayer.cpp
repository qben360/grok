...
bool IOPlayer::loadPlayer(Player* player, const std::string& name, bool preload /*= false*/)
{
	...
	int64_t redSkullSeconds = lastSkullTime - std::time(NULL);
	if(redSkullSeconds > 0){
		player->lastSkullTime = (redSkullSeconds + 2)*1000;
...
bool IOPlayer::savePlayer(Player* player, bool shallow)
{
	...
	if(player->lastSkullTime > 0){
		redSkullTime = std::time(NULL) + player->lastSkullTime/1000;
	}
	
	query << ", `skull_time` = " << redSkullTime;
#endif
