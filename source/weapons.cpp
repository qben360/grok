...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	if(hitChance == -1){
		...
		if(maxHitChance == 75){
			...
			switch(distance){
				case 0: chance = (uint32_t)((float)0.3 * std::min(skill, (uint32_t)74)) + 1; break;
				case 1: chance = (uint32_t)((float)0.4 * std::min(skill, (uint32_t)74)) + 1; break;
				...
				case 5: chance = (uint32_t)((float)0.4 * std::min(skill, (uint32_t)74)) + 1; break;
