...
int Items::loadFromOtb(std::string file)
{
	...
	if(f.getProps(node,props)){
		...
		if(!props.GET_UINT8(attr)){
			std::cout << "INVALID FORMAT1" << std::endl;
			return ERROR_INVALID_FORMAT;
		}
		if(attr == ROOT_ATTR_VERSION){
			if(!props.GET_VALUE(datalen)){
				std::cout << "INVALID FORMAT2" << std::endl;
				return ERROR_INVALID_FORMAT;
			}
			if(datalen != sizeof(VERSIONINFO)){
				std::cout << "INVALID FORMAT3" << std::endl;
				return ERROR_INVALID_FORMAT;
			}
		VERSIONINFO vi;
	...
	while(node != NO_NODE){
		PropStream props;
		if(!f.getProps(node,props)){
			std::cout << "f.getProps(node,props) 242" << std::endl;
			return f.getError();
		}
		...
		switch(type){
			...
			case ITEM_GROUP_DEPRECATED:
				break;
			default:
				//std::cout << "INVALID FORMAT KURWA" << std::endl;
				//return ERROR_INVALID_FORMAT;
				break;
		}
		...
		//read 4 byte flags
		if(!props.GET_UINT32(flags)){
			std::cout << "props.GET_UINT32(flags)" << std::endl;
			return ERROR_INVALID_FORMAT;
		}
		...
		while(props.GET_UINT8(attrib)){
			...
			if(!props.GET_UINT16(datalen)){
				std::cout << "!props.GET_UINT16(datalen)" << std::endl;
				delete iType;
				return ERROR_INVALID_FORMAT;
			}
			switch(attrib){
			case ITEM_ATTR_SERVERID:
			{
				if(datalen != sizeof(uint16_t)) {
					std::cout << "datalen != sizeof(uint16_t)" << std::endl;
					return ERROR_INVALID_FORMAT;
				}

				uint16_t serverid;
				if(!props.GET_UINT16(serverid)){
					std::cout << "!props.GET_UINT16(serverid)" << std::endl;
					return ERROR_INVALID_FORMAT;
				}

				/*if(serverid > 20000){
					std::cout << "SERVERID > 20000" << std::endl;
					return ERROR_INVALID_FORMAT;
				}*/
				iType->id = serverid;
				break;
			...
			{
				if(datalen != sizeof(uint16_t)){
					std::cout << "CLIENT datalen != sizeof(uint16_t)" << std::endl;
					return ERROR_INVALID_FORMAT;
				}

				uint16_t clientid;
				if(!props.GET_UINT16(clientid)){
					std::cout << "CLIENT !props.GET_UINT16(clientid)" << std::endl;
					return ERROR_INVALID_FORMAT;
				}
			...
			case ITEM_ATTR_LIGHT2:
			{
				if(datalen != sizeof(lightBlock2)) {
					std::cout << "DATALEN LIGHT2" << std::endl;
					return ERROR_INVALID_FORMAT;
				}
			...
			default:
				...
				if(!props.SKIP_N(datalen)) {
					std::cout << "UNKNOWN ATTRIB !! !! !! !!" << std::endl;
					return ERROR_INVALID_FORMAT;
				}
				break;
			}
		...
		//std::cout << "Loading items.otb - serverId: [" << iType->id << "]" << "clientId: [" << iType->clientId << "]" << std::endl;
			...
			if(readXMLInteger(itemNode, "id", intValue)){
				...
				while(itemAttributesNode){
					if(readXMLString(itemAttributesNode, "key", strValue)){
						...
						else if(asLowerCaseString(strValue) == "toporder"){
							if(readXMLInteger(itemAttributesNode, "value", intValue)){
						...
						else if(asLowerCaseString(strValue) == "showattributes"){
							if(readXMLInteger(itemAttributesNode, "value", intValue)){
						...
						else if(asLowerCaseString(strValue) == "suppressparalyze"){
							if(readXMLInteger(itemAttributesNode, "value", intValue)){
						...
						else if(asLowerCaseString(strValue) == "increasemagicpercent")
						{
							if(readXMLInteger(itemAttributesNode, "value", intValue))
							it.abilities.increasemagicpercent = (intValue);
						}
